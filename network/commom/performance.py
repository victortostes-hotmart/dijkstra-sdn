...
def build_server_command(server, protocol, folder):
  if protocol is 'UDP':
    return 'iperf -s -e -u -i 1 -f m -O 5 > %s/%s_%s.txt' % (folder, protocol, server.name);
  
  elif protocol is 'TCP':
    return 'iperf -s -i 1 -f m -O 5 > %s/%s_%s.txt' % (folder, protocol, server.name);

def build_client_command(server, protocol, bw, timeInSecs):
  if protocol is 'UDP':
    return 'iperf -c %s -t %d -i 1 -e -u -b %sM -O 5' % (server.IP(), timeInSecs, bw);
  
  elif protocol is 'TCP':
    return 'iperf -c %s -t %d -i 1 -b %sM -O 5' % (server.IP(), timeInSecs, bw);
  

def start_test(serverCommands, clientCommands, timeInSecs):
...
def pairs_test(network=None, protocol='TCP', timeInSecs=15, bw=1, folder=None):
  ...
  start_test(serverCommands, clientCommands, timeInSecs);
